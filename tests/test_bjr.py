...
def test_obtenir_deviner_utilisateur(mock_input: patch) -> None:
    devinette: int = obtenir_deviner_utilisateur()
    assert devinette == 10


@patch("builtins.input", side_effect=["30", "abc", "15"])
def test_obtenir_deviner_utilisateur_invalid_input(mock_input: patch) -> None:
    devinette: int = obtenir_deviner_utilisateur()
    assert devinette == 15
...
def test_jouer_partie(
    mock_donner_indice: patch, mock_generer_nombre_aleatoire: patch, mock_input: patch
) -> None:
...
def test_demander_si_rejouer_oui(mock_input: patch) -> None:
...
def test_jeu_devinette(mock_jouer_partie: patch, mock_input: patch) -> None:
    with patch("builtins.print") as mock_print:
        jeu_devinette()
